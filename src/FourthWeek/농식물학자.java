		...
		if(생존자.남은행동수==0) {
			...
			if(생존자.포만감<=19) {
				...
				if(Math.random()<=농사성공확률){
					농사능력각성도++; System.out.println("농사에 성공하여 농사능력각성도가 1증가했습니다. 각성도가 20이상이 될시 농사능력이 2->4로 증가되어 적용되고 농사성공확률 70%->90%로 증가합니다.");
					System.out.println("현재 농사능력각성도 :" + 농사능력각성도); 
					if(농사능력각성도>=20) {
						농사능력각성발휘 = true;
					}
